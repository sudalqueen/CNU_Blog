...
const Write = () => {
  ...
  const handleChageTitle = (event: ChangeEvent<HTMLInputElement>) => {
    setTitle(event?.target.value);
  };

  const handleChageContent = (event: ChangeEvent<HTMLTextAreaElement>) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{height: 'calc(100% - 4rem)', paddingBottom: '4rem'}}>
        <TitleInput placeholder="제목을 입력하세요" value={title} onChange={handleChageTitle} />
        <TagSelect value={tag} onChange={handleChangeTag} placeholder={'태그를 선택하세요'}>
        </TagSelect>
        <Editor value={content} onChange={handleChageContent} placeholder='내용을 입력하세요' />
      </div>
      <BottomSheet>
        <Link to="/">
